...
export default function ListCard({ data, showModal }) {
  const { saves, addSaves, removeSaves } = useSave();

  return (
    <>
      {/* start */}
      <div className="row d-flex flex-column flex-sm-row flex-wrap gap-3 px-md-5 w-100 " style={{ margin: "auto" }}>
        {data.map((v, i) => {
          return (
            <Link
              key={v.id}
              href={`/art-gallery/artwork/${v.id}`}
